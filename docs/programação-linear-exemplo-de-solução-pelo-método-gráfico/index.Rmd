...
```{r tib, echo=FALSE, message=FALSE, warning=FALSE}

library(dplyr)
library(kableExtra)

df <- dplyr::tibble(' ' = c("Unidades", "Pasta química", "Pasta mecânica"), 
               'Preço de venda' = c("[R$/t]","100", "200" ), 
               "Poulição" =  c("[dbo/t]","1", "1.5"),
                "Mão de obra" =  c("[homem-dia/t]","1", "1"),
               "Capacidade instalada" = c("[t/dia]"," 300", "200"))
df %>%
kbl() %>%
...
```{r canoni, echo=FALSE, out.width = '100%', fig.align='center', fig.pos='h'}
...
```{r equacoes, include=TRUE, echo=TRUE, eval=TRUE}
...
```{r fact, include=TRUE, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r grad, include=TRUE, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r offset, include=TRUE, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r lpsolve, include=TRUE, echo=TRUE, eval=TRUE, warning=FALSE, message= FALSE}
...
```{r sol_graf, include=TRUE, echo=FALSE, eval=TRUE, warning=FALSE, message= FALSE, results='asis'}

# Plot da solução ótima

p5 <- p4 + geom_point(aes(x = solution[1], y = solution[2]), color = "red", size = 4) +
  labs(subtitle = "Solução gráfica")
